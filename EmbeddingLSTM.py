...
# Time: 2 h 52 min  29s
# 七绝
# 轮数: 20/20...  训练步数: 13377...  训练误差: 4.2881...  4.9398 sec/batch
# 轮数: 20/20...  训练步数: 13378...  训练误差: 4.3539...  5.0585 sec/batch
# 轮数: 20/20...  训练步数: 13379...  训练误差: 4.3229...  4.9791 sec/batch
# 轮数: 20/20...  训练步数: 13380...  训练误差: 4.2585...  5.0303 sec/batch
# Time: 5 h 37 min  5s
import time
import numpy as np
...
class LstmAnna():
    def __init__(self, name='七律', isChinese = False, num_steps = 256, epochs = 20, batch_size = 64):
        ...
        self.wordnum = 0
        self.top_n = 10
        self.checkpoints = 'model/' + self.name + '/'
    ...
    def runLSTM(self):
        ...
        print("训练完成")
    def pick_top_n(self, preds,mode=1):    # 从预测结果中选取前top_n个最可能的字符
        p = np.squeeze(preds)
        # 将除了top_n个预测值的位置都置为0
        ...
        if mode:
            while self.vocab_times[self.int_to_vocab[c]] < 250 or (self.int_to_vocab[c] in self.samples and self.int_to_vocab[c] not in ['，','。','\n']) \
                    or (self.samples[-1] in ['，','。'] and self.vocab_times[self.int_to_vocab[c]] in ['，','。']):
            ...
            return c
        return c
    def pick_top_n2(self, preds):    # 从预测结果中选取前top_n个最可能的字符
        p = np.squeeze(preds)
        # 将除了top_n个预测值的位置都置为0
        p[np.argsort(p)[:-self.top_n]] = 0
        # 归一化概率
        ...
        # 删除出现次数太少的情况
        times = 0
        while self.vocab_times[self.int_to_vocab[c]] < 250 or self.int_to_vocab[c] in self.samples or self.int_to_vocab[c] in ['，','。','\n','？'] or self.int_to_vocab[c] in self.poem:
            # print(self.int_to_vocab[c] + "太少了或者出现过")
            call = np.random.choice(len(self.vocab), 5, p=p,replace=False)
            c = call[0]
            ii = 1
            while self.int_to_vocab[c] in ['，','。','\n','？']:
                c = call[ii]
                ii += 1
            times += 1
            if times > 100:
                # print(self.int_to_vocab[c])
                break
        return c
    def changeChar(self,c1):
        if c1 == '閒':
            c1 = '闲'
        if c1 == '鴈':
            c1 = '雁'
        ...
        if c1 == '羨':
            c1 = '羡'
        return c1
    def sample(self,mode=0,mode2=1):
        ...
        with tf.Session() as sess:
            ...
            for c in self.prime:
                ...
                preds, new_state = sess.run([model.prediction, model.final_state],
                                            feed_dict=feed)
            if mode == 1:
                c = self.pick_top_n2(preds)
            else:
                c = self.pick_top_n(preds,mode2)
            # 添加字符到samples中
    ...
    def writePoem(self):
        ...
        self.wordnum = numWords[self.name] * 2
        # print("开始在" + self.checkpoints + "目录模型下\n写" + str(numWords) + "个词" + "\n以" + self.prime + "开始")
        while 1:
            self.samp = self.sample()
            # print(self.samp)
            poem = self.samp.split('\n')
            for item in poem:
                if(len(item) == numWords[self.name]):
                    self.poem = item
                    # print(str(len(item)) + ':', end='')
                    # print(poem)
                    self.printPoem(item)
                    return item
    def printPoem(self, item):
        print(item[:(len(item) // 2)])
        print(item[(len(item) // 2):])
        print()
    def writePoemCangTou(self, prime = ""):
        primeall = prime
        numWords = {
            "七律": 7,
            "五律": 5,
        ...
        for i in range(numTimes[self.name] -len(primeall)):
            with open('data/' + self.name + '-Word.txt', 'r', encoding='utf-8') as f:
                all = f.read()
                # all = all.split('\n')
                primeall += random.choice(all)
        self.poem = ""
        Biao = ['，','。']
        self.wordnum = numWords[self.name] - 2
        # print("开始在" + self.checkpoints + "目录模型下\n写" + str(numWords) + "个词" + "\n以" + self.prime + "开始")
        for i in range(numTimes[self.name]):
            self.prime = primeall[i]
            self.poem += self.sample(mode=1)
            self.poem += Biao[i % 2]
        self.printPoem(self.poem)
        return self.poem
...
if __name__ == '__main__':
    #  模型初始化
    lstm = LstmAnna(name='七绝', isChinese=True,
                    epochs=20,
                    batch_size=64)
    ...
    # lstm.writeAll(prime='The', numWords=400)
